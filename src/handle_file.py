...
from src.ipalbum_exception import IPAlbumException, IPAlbumError
...
def read_avm(avm_dir_path: str, avm_file_name: str = '') -> (str, str):
    ...
    if avm_file_name != '':
        ...
        if not os.path.exists(avm_file_path):
            raise IPAlbumException(IPAlbumError.other_error(avm_file_path + ' not exist'))
...
def read_abi(abi_dir_path: str, abi_file_name: str) -> dict:
    if not os.path.isdir(abi_dir_path):
        raise IPAlbumException(IPAlbumError.other_error('build folder not exist, please compile first'))
    abi_file_path = os.path.join(abi_dir_path, abi_file_name)
    if not os.path.exists(abi_file_path):
        raise IPAlbumException(IPAlbumError.other_error('abi config file not exist'))
